...
################################################################################################################################
#																															   #
# This program aims to estimate the duplication level within the reads by using adapter-trimmed, but quality-unfiltered reads! #
#																															   #
# It is used for extracting compressed FASTQ files, performing alignment using bwa mem on quality-unfiltered reads,			   #
...
# Additionally, the time for each step as well as the total time is recorded for every sample.								   #
#																															   #
################################################################################################################################
...
OUT_PATH_BAM=../../data/alignment_data_duplicates
mkdir -p $OUT_PATH_BAM
# Creating and specifying the directory for the alignment statistics
OUT_PATH_STAT=../../analysis/alignment_data_duplicates
...
for folder in $IN_PATH/*; do
	...
	unzip_start=`date +%s`
	for comp in ${folder}/*clean_both.fastq.bz2; do
		bzip2 -dk $comp
	done
	unzip_end=`date +%s`
	...
	echo "Alignment and compression done for sample ($count of $TOTAL)"
